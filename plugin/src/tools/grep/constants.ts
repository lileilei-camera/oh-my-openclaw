...
function getOpenClawBundledRg(): string | null {
...
export function resolveGrepCli(): ResolvedCli {
  ...
  const bundledRg = getOpenClawBundledRg();
  if (bundledRg) {
...
export async function resolveGrepCliWithAutoInstall(): Promise<ResolvedCli> {
  ...
  // Auto-install not implemented in OpenClaw
  // Return current (will use grep fallback or fail gracefully)
  return current;
}

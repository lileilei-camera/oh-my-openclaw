...
import type { GrepOptions, GrepMatch, GrepResult, CountResult } from './types.js';
...
export async function runRgCount(options: Omit<GrepOptions, 'context'>): Promise<CountResult[]> {
  ...
  try {
    ...
    if (e.killed) {
      throw new Error(`Count search timeout after ${timeout}ms`);
    }
    // grep returns exit code 1 when no matches found (not an error)
    if (e.status === 1 && !e.stdout) {
      return [];
    }
    throw new Error(`Count search failed: ${e.message || String(e)}`);
  }
}

// ast-grep constants - copied from oh-my-opencode verbatim
...
export function findSgCliPathSync(): string | null {
  const binaryNames = process.platform === "win32" ? ["ast-grep.exe", "sg.exe"] : ["ast-grep", "sg"]
  ...
  const commonPaths = [
    process.platform === "darwin" ? "/opt/homebrew/bin/ast-grep" : null,
    process.platform === "darwin" ? "/usr/local/bin/ast-grep" : null,
...
export function getSgCliPath(): string {
  ...
  // Default to ast-grep (npm package name), fallback to sg
  return "ast-grep"
}
...
export function formatEnvironmentCheck(result: EnvironmentCheckResult): string {
  ...
  if (result.napi.available) {
    ...
    lines.push(`  Install: npm install -g @ast-grep/napi`)
  }

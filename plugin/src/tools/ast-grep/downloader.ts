// ast-grep downloader - copied from oh-my-opencode verbatim, adapted for OpenClaw
...
export function getCacheDir(): string {
  if (process.platform === "win32") {
    ...
    return join(base, "oh-my-openclaw", "bin")
  }
...
export async function downloadAstGrep(version: string = DEFAULT_VERSION): Promise<string | null> {
  ...
  if (!platformInfo) {
    console.error(`[oh-my-openclaw] Unsupported platform for ast-grep: ${platformKey}`)
    return null
  }
  ...
  console.log(`[oh-my-openclaw] Downloading ast-grep binary...`)
  ...
  try {
    ...
    // Extract zip using unzip command (or Bun's extractZip if available)
    try {
      execSync(`unzip -o "${archivePath}" -d "${cacheDir}"`, { stdio: "ignore" })
    } catch {
      // Try alternative extraction methods
      try {
        execSync(`tar -xf "${archivePath}" -C "${cacheDir}"`, { stdio: "ignore" })
      } catch {
    ...
    console.log(`[oh-my-openclaw] ast-grep binary ready.`)

    return binaryPath
  } catch (err) {
    console.error(
      `[oh-my-openclaw] Failed to download ast-grep: ${err instanceof Error ? err.message : err}`
    )
    return null
  }
}

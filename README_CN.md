...
| 智能体 | 个性 |
|-------|------|
| **Planner** | 战略规划师。在你开始之前不会让你知道你想要什么。 |
| **Delegate** | 指挥家。不演奏乐器。确保完美和谐。 |
| **Coder** | 工作马。专注。自律。不完成不停止。 |
| **Expert** | 工匠。给他一个难题，一小时后回来。 |
| **Architect** | 架构顾问。只读。昂贵。值得每个 token。 |
| **Explorer** | 侦察兵。知道代码库中的一切在哪里。 |
| **Researcher** | 研究员。文档、知识、上下文 —— 按需。 |
| **Advisor** | 差距发现者。发现其他人都错过的东西。 |
| **Reviewer** | 评论家。你的计划有漏洞 —— Reviewer 会发现它们。 |
| **Looker** | 眼睛。截图、PDF、UI 审查 —— 看到文本看不到的。 |
